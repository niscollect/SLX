...
import { callParse } from "../service/testing/parse_caller.js";
import { fileRedirector } from "../service/fileRedirector.js";
...
export async function crawl(rootDir)
{
    ...
    for (const file of arrayOfAllFiles)
	{
        console.log("Processing file: ", file);

        let fileJsonRes;
        try {
            fileJsonRes = await fileRedirector(file); // sequential async
            // we'll do sequential async for now, and then later improve it to batched parallelism
        } catch (error) {
            console.error("Encountered an error before processing the file:", error);
        }
        //* console.log(file, " - file JSON result:", fileJsonRes);
        console.log("-----------------");
        console.log("=================");

        if(fileJsonRes)
            combinedJson = { ...combinedJson, ...JSON.parse(fileJsonRes) };
    }

    console.log("\n\n ------------------------------------- \n\n")
    console.log("Final array of JSONs:", combinedJson);

    //* combinedJson is the final JSON object containing all routes from all files, i.e. all the navigable surfaces

    //* now we need to add attributes to each object, to make the JSON rich enough for predictions/matching
...
//? convert the JSON objects to JSON array and concat using .concat